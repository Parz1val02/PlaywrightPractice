...
export class TopMenuPage {
    ...
    constructor(page: Page) {
		...
		this.pythonLink = page.getByLabel('Main', { exact: true }).getByRole('link', { name: 'Python' })
		this.dotnetLink = page.getByLabel('Main', { exact: true }).getByRole('link', { name: '.NET' });
		...
		this.appearance = page.getByRole('button', { name: 'Switch between dark and light' });
    }
	...
	async testSearchModal(){
		...
		await this.page.getByRole('searchbox', { name: 'Search' }).fill('languages');
		await this.page.getByRole('option', { name: 'Supported languages', exact: true }).getByRole('link').click();
